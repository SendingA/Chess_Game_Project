...
//        statusLabel.setText("Current Player:\n" + ChessGameFrame.gameController.getChessboard().getCurrentColor());
        count++;
        ChessGameFrame.gameController.record();
    ...
    public int loadGame(List<String> chessData) {
        ...
        initiateEmptyChessboard();
        if (chessData.size() < 9) {
            if ((chessData.get(chessData.size() - 1).charAt(0) != 'b') && (chessData.get(chessData.size() - 1).charAt(0) != 'w')) {
                return 103;
            } else {
                return 101;
            }
                ...
                for (int j = 0; j < 8; j++) {
                    switch (chessData.get(i).charAt(j)) {
                        ...
                        case 'p':
                            ...
                            break;
                        case '_':
                            initPawnOnBoard(i, j, ChessColor.NONE);
                            break;
                        default:
                            return 102;
                    }
